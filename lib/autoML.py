...
"""
from lib.Utils.Display import color_print, print_title1
from lib.Utils.Decorators import timer
from lib.Audit.Audit_Dataset import audit_dataset
from lib.Audit.Get_Outliers import get_cat_outliers, get_num_outliers
from lib.Preprocessing.Date_Data import all_to_date, date_to_anc
from lib.Preprocessing.Missing_Values import fill_all_num, fill_all_cat
from lib.Preprocessing.Outliers import process_cat_outliers, process_num_outliers
from lib.Preprocessing.Categorical_Data import dummy_all_var
from lib.Modelisation.Tree_based.HyperOpt import *
...
class AutoML(pd.DataFrame):
    ...
    * models selection
    ...
    @timer
    def train_predict(self, n_comb=10, comb_seed=None, verbose=1):
        """
        ...
        hyperopt.train(df_train, self.target, print_level=verbose)
        """
        color_print('applying models')
        # Application des modèles sur X_test :
        dict_res_model = hyperopt.predict(df_test, self.target, print_level=verbose)

        # selection best model
        color_print('best model selection')
        best_model_idx, _ = hyperopt.get_best_model(dict_res_model, metric='F1', delta_auc=0.03, print_level=verbose)

        if verbose > 0:
            print_title1('best model : ' + str(best_model_idx))
            if verbose > 1:
                print(hyperopt.model_res_to_df(dict_res_model, metric='F1'))
