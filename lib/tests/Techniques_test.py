...
df_iris_binary = pd.read_csv('~/automl/Iteration3/iris_binary.csv')
...
class Test_Bagging(unittest.TestCase):
    ...
    def test_get_params(self):
        ...
        self.assertEqual(param_dict['remise'], True)
        self.assertEqual(param_dict['list_model'], list())
    
    #
    def test_set_params(self):
        self.bagging.set_params({'niter' : 5, 'remise' : False, 'test' : 'invalid'})
        param_dict = self.bagging.get_params()
        self.assertEqual(param_dict['niter'],5)
        self.assertEqual(param_dict['pos_sample_size'], 1.0)
        self.assertEqual(param_dict['remise'], False)
        self.assertEqual(param_dict['list_model'], list())
