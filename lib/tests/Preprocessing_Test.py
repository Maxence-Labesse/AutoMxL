...
from lib.Preprocessing.Categorical_Data import *
from lib.Preprocessing.Date_Data import *
from lib.Preprocessing.Outliers import *
from lib.Preprocessing.Missing_Values import *
...
class Test_Missing_values(unittest.TestCase):
    ...
    def test_fill_num(self):
        df_fill_med = fill_num(self.df, 'Age', method='median', top_var_NA=True, verbose=0)
        df_fill_zero = fill_num(self.df, 'Height', method='zero', top_var_NA=True, verbose=0)
        df_fill_mean = fill_num(self.df, 'Height', method='mean', top_var_NA=True, verbose=0)
    ...
    def test_fill_cat(self):
        df_fill_nr = fill_cat(self.df, 'Name', method='NR', top_var_NA=True, print_level=0)
...
class Test_Date_preprocessing(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.df_to_anc = date_to_anc(self.df_to_date, var_list = ['American_date_nai', 'Date_nai'], date_ref='27/10/2010', verbose=0)
    
    def test_all_to_date(self):    
        self.assertEqual(self.df_to_date['American_date_nai'].dtype,'datetime64[ns]')
        self.assertEqual(self.df_to_date['Date_nai'].dtype,'datetime64[ns]')
...
class Test_Categorical(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.df_dummy_pref = dummy_all_var(self.df, var_list=['Eyes','Sexe'], prefix_list=['Ey','Sx'], keep=True, verbose=0)
...
class Test_Outliers(unittest.TestCase):
    ...
    def setUp(self):
        self.df = pd.DataFrame(data)
        self.df_perc_bind = remove_bind(self.df,'Hair', method='percent', threshold=0.30, print_level=0)
        self.df_nb_bind = remove_bind(self.df,'Hair', method='number', threshold=1, print_level=0)
        self.df_nbvar_bind = remove_bind(self.df,'Hair', method='nbvar', threshold=2, print_level=0)
        self.df_process_cat = process_cat_outliers(self.df, ['Name','Hair'], method="percent", threshold=0.30, verbose=0)
