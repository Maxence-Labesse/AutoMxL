...
def fill_cat(df, var, method='NR', top_var_NA=True):
    """
    ...
    if method in ['NR', 'reg']:
        ...
        if (top_var_NA == True and df[var].isna().sum() > 0):
            var_na = 'top_NA_' + var
            df_local[var_na] = 0
            df_local.loc[df_local[var].isna(), var_na] = 1
...
def fill_all_cat(df, var_list=None, method='NR', top_var_NA=True, verbose=1):
    """
    ...
    > top_var_NA : boolean (Default : True)
        if True, create a boolean column to identify replaced observations
    > verbose : int (0/1) (Default : 1)
        get more operations information
    ...
    # fill Na values for each feature in var_list
    for j in var_list:
        df_local = fill_cat(df_local, j, method, top_var_NA)
